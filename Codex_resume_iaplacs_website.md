...
- Local verification passed: `git diff --check`. Pending after this entry: commit/push the sentinel correction, force-render current Ningxia regional/national mosaics, and visually verify the dry panels return to white without a label.
- Preserve the user-owned unstaged `.gitignore` entry for `.tmp`; do not commit or discard it.
