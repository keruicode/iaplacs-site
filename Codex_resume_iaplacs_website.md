...
## 2026-07-23 Tianhe External Network Diagnostic

- Read-only test from `th-hpc4-ln1` found that Tianhe currently has no configured DNS resolver: `/etc/resolv.conf` contains only `# Generated by NetworkManager`, and `getent ahosts github.com` plus `getent ahosts iaplacs-forecast-images-hk.oss-cn-hongkong.aliyuncs.com` returned no addresses.
- Consequently, `curl` to GitHub and the IAPLACS OSS hostname both failed with `Could not resolve host`.
- The default route exists (`192.168.4.1` via `eno4`) and direct HTTPS to public IP `1.1.1.1` returned HTTP 404, so outbound IP connectivity is available. This is a DNS configuration issue, not an OSS ACL/object problem.
- No Tianhe network configuration was changed. Obtain approved cluster DNS server addresses or a required outbound proxy from Tianhe support, configure them through the system/NetworkManager policy, then rerun the hostname and OSS-object checks.

## 2026-07-23 Tianhe Git/OpenSSL Compatibility

- User's `git clone https://github.com/...` failure was diagnosed as an ABI collision, not a GitHub repository issue. The active `LD_LIBRARY_PATH` injects Spack OpenSSL `1.1.1h`; system `/usr/libexec/git-core/git-remote-https` then loads system `libssh.so.4` against that incompatible OpenSSL and fails on `EVP_KDF_ctrl`.
- Added source helper `tools/tianhe_system_git.sh` and installed it on Tianhe as executable `/fs1/home/sunjm/kerui/bin/git-system`. It unsets `LD_LIBRARY_PATH` and `LD_PRELOAD`, then executes `/usr/bin/git` with a system-only `PATH`.
- Verification: `git-system --version` returns `git version 2.27.0`; `git-system ls-remote https://github.com/joshmedeski/dotfiles.git` no longer has the OpenSSL/libssh symbol error. It currently stops only at `Could not resolve host: github.com`, matching the separate DNS diagnosis.
- After Tianhe DNS/proxy is configured, use:
  ```bash
...
## 2026-07-23 Tianhe Account Migration to junzhang

- User changed the Tianhe target account to `junzhang@192.168.10.50`. The requested final destination is `/fs2/home/junzhang/kerui`; `/fs2/home/junzhang/zhoubj` remains untouched for later WORK/WORK_nx/WORK_yn/WORK_tc setup.
- Read-only inventory confirmed the old source `sunjm@192.168.4.11:/fs1/home/sunjm/kerui` was 26 MB and contained `bin/git-system` plus `iaplacs-runtime` releases, backup, and `current` symlink. The new account had 2.1 PB free on `/fs2` and the final `kerui` directory was empty.
- Completed a Mac-relayed archive migration using transfer ID `20260723_175106_8936`. The source archive was 15 MB; its local SHA-256 was `daed5a5f9a1df327ed83413e9d08621e366241dff4fa6ceea0e01b4e6921fc81`. The same archive passed SHA-256 verification on the new account before extraction. The staged extraction was validated for executable `bin/git-system`, `iaplacs-runtime/current`, and the Ningxia publisher, then replaced only the confirmed-empty `/fs2/home/junzhang/kerui` directory.
- Verified new target files include the 14,545,508-byte province SHP, 7,650,427-byte runtime backup, 55 regular files, executable `/fs2/home/junzhang/kerui/bin/git-system`, and the active `iaplacs-runtime/current` symlink. `/fs2` reports `du` block usage differently (72 KB) but the expected logical file sizes are present.
- Removed all transfer intermediates: source `/tmp/iaplacs-kerui-20260723_175106_8936.{tar.gz,tar.gz.sha256}`, Mac `/private/tmp/iaplacs-kerui-junzhang.UsajEVeg`, and target `/fs2/home/junzhang/.iaplacs-kerui-incoming-20260723_175106_8936/`. SFTP checks confirm each is absent.
- Added `tools/relay_tianhe_kerui_account.sh` and documented it in `docs/tianhe-migration.md`. It protects the fixed source/target paths, refuses a nonempty target, relays through a Mac temporary directory, checks SHA-256, and removes only its own temporary archive/staging paths. Its checksum file now uses only the archive filename, not an absolute source path, so verification works on the Mac and new account.
- Do not install a Tianhe cron yet. The next required input is the real new-account locations and module/Slurm environment for the future `zhoubj/WORK*` output trees. The migrated runtime is a recovery/staging base, not an enabled forecast workflow.
- The migration relay/docs/handoff were rebased onto automated forecast commit `539ba46 Update WORK_nx summary 20260723_00` and pushed as `d8ca70a Add Tianhe account migration relay`. The only local working-tree change remains the intentionally excluded `.gitignore` entry for `.tmp`.

## 2026-07-23 New Tianhe Account Environment Inventory

- New account host is `th-ex-ln0`. `/usr/bin/crontab` exists and `crontab -l` reports no crontab for `junzhang`; do not install one yet.
- System Python is `/usr/bin/python3` 3.6.8 and `python` is absent. It is not suitable as the operational scientific runtime. The existing usable scientific interpreter is `/fs2/home/junzhang/zhoubj/conda_envs/wrf-scripts/bin/python` 3.10.20; it successfully imported `numpy 2.2.6` and `netCDF4 1.7.4`.
- `zhoubj/env.sh` only prepares the WRF compiler/MPI/NetCDF environment (Intel 19.0.4, Intel MPI 2019.4.243, NetCDF 4.6.2). It does not activate a Conda environment or create a `python` command. Use the explicit Conda Python path or explicitly activate the required environment in future automation.
- Slurm is available: `/usr/bin/sbatch` and `/usr/bin/squeue` are both 22.05.2. The module command is available and no modulefiles are loaded by default. Existing WRF run scripts show `#SBATCH` and `srun` usage. Note that archived `sg*.sh` scripts reference Intel 2021.3.0/NetCDF 4.4.1 while `env.sh` references the older 2019.4.243/NetCDF 4.6.2 stack; do not hard-code either until a real WRF run validates the currently supported stack.
- `/usr/bin/git` and `/fs2/home/junzhang/kerui/bin/git-system` both report Git 2.27.0, so the OpenSSL ABI wrapper is installed and executable. GitHub HTTPS is not yet usable on this new login node: `git-system ls-remote https://github.com/keruicode/iaplacs-site.git` fails at `Could not resolve host: github.com`. This is a DNS/egress issue separate from Git itself; no GitHub publisher cron should be enabled until it is resolved or a Mac relay is used.
- `zhoubj` currently contains `env.sh`, `bkenv.sh`, `wrf-scripts`, `conda_envs/wrf-scripts`, `WRF/wrf413_3d`, and `WORK`. The user expects `WORK`, `WORK_nx`, `WORK_yn`, and `WORK_tc` there later; keep `kerui` as the migrated runtime/publisher base and reference these paths only after they are finalized.

## 2026-07-24 Tianhe Website Test and Source Switch

- User confirmed Tianhe model testing should begin, with a website source switch between 寰 and 天河 and Tianhe services handled first. Existing user changes in `index.html`, all three service pages, `app.js`, `styles.css`, and `tools/optimize_forecast_images.sh` were preserved and completed.
- Added source buttons to `/`, `/ningxia/`, `/shangrao/`, and `/airpots/`. The switch uses `localStorage`, defaults to 寰, reads the existing `data/current` catalog for 寰, and reads `data/tianhe/current` for 天河. Relative asset paths work for both root and nested routes.
- Tianhe inventory found current run `2026072306`: `WORK_nx` has `Precip_hourly_WRF_AllRain_T01_T48_InitUTC_2026-07-23_06_00.png`; `WORK_yn` has `Precip_hourly_YunnanDomain_TargetT07_T48_ActualT07_T48_InitUTC_2026-07-23_06_00.png` and `Precip_hourly_YunnanLocal_TargetT07_T48_ActualT07_T48_InitUTC_2026-07-23_06_00.png`. `WORK_tc` is absent and no Tianhe Shangrao product was found.
- Relayed the three Tianhe PNGs through the Mac into `data/tianhe/current/maps`, optimized them to six WebP/preview WebP assets totaling about 3.4 MB, then deleted only the local PNG copies. The original PNGs remain on Tianhe.
...
- Updated `tools/rain_worknx_ningxia_hour_bjt.ncl` and `tools/rain_worknx_national_hour_bjt.ncl`: trace/zero rain uses `-1.0`, below the first `0 mm` contour, so it renders with the existing white below-range class rather than missing. `cnNoDataLabelOn=False` is explicit alongside the existing constant/info label suppression. This does not alter time selection, thresholds, or BJT labels.
- First deployment/re-render completed and exposed a palette detail: placing trace rain at exactly `0.0` rendered it in the `0--1.5 mm` green bin. That data commit must not remain the final state; it was visually caught and corrected to the below-range `-1.0` sentinel before the next publication.
- Final correction committed as `858d721 Keep dry Ningxia panels white`, then pulled onto Tianhe. The one-time force render completed and pushed data commit `770a660 Publish Tianhe forecasts worknx_summary_2026072418 airport_yunnan_2026072412 shangrao_2026072412`. Visual inspection of the final Ningxia preview verified all dry panels are white with boundaries/colorbars intact and no `NO CONTOUR` text; precipitation panels retain the original green-to-blue scale. NCL still logs a harmless constant-field diagnostic for completely dry panels, but its visual label is suppressed.
- The force run also created the `WORK_nx/2026072418` 39 MB precipitation backup and safely deleted the already backed-up `WORK_nx/2026072412` full WRF directory, releasing about 130 GB. It did not delete the non-stable `2026072406` directories or the still-running `WORK`/`WORK_yn` 2026072418 products.
- Public verification passed: `https://iaplacs.xyz/data/tianhe/current/forecast-runs.json?v=770a6602` reports latest Ningxia `tianhe_worknx_summary_20260724_18`, run time `2026-07-25T02:00:00+08:00`; the corrected regional WebP returns `HTTP 200`, `image/webp`, and `Content-Length: 697144`.
- Preserve the user-owned unstaged `.gitignore` entry for `.tmp`; do not commit or discard it.
...
- Deployment and validation completed. Code commit `bf3a5fd Back up precipitation before pruning Tianhe WRF runs` is pushed and the Tianhe active checkout fast-forwarded to `bf3a5fd`. `--dry-run` reported only planned backups and explicitly retained `2026072418`/`2026072406` as non-stable. A locked real publisher run then created and validated all three `2026072412` backups: `WORK` 24 MB, `WORK_nx` 36 MB, and `WORK_yn` 32 MB. No WRF directory was deleted because each family has no newer stable completed run yet. The existing cron at `7,22,37,52 * * * *` invokes this cleanup-enabled publisher; no additional overlapping cron was added.
- Preserve the user-owned unstaged `.gitignore` entry for `.tmp`; do not commit or discard it.
