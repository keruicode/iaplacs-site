...
网站页面、前端逻辑和预报清单放在 GitHub Pages；大图优先从阿里云 OSS 加载；IAP 服务器定时生成图像、上传 OSS、更新 `data/current` 清单并推送到 GitHub。
...
页面主图优先使用 WebP，保证首次渲染快。放大查看和下载优先使用同一张图的 PNG 原图；如果 catalog 没有显式 `full_file` 字段，前端会按 `.webp -> .png` 自动推断，失败时回退 WebP。

GitHub 仓库里的 `data/current/maps/*` 仍然有图像副本，主要用于历史追踪、fallback 和 catalog 生成，不是浏览器的首选大图来源。

## IAP 服务器发布流程

服务器侧的预报发布大致是：

1. 生成新的 `worknx_summary_*` 或 `wrf_montage_*` 目录。
2. 写入 PNG 图、WebP 图和必要的 `manifest_fragment.json`。
3. 上传 PNG/WebP 到 OSS，并确认公共 URL 可读。
...
本地不要随手提交整个 `data/current`。这个目录经常由 IAP 服务器自动更新，`lazygit` 里看到的大量图片新增、删除、修改，很多只是服务器发布流和本地旧工作树之间的差异。
