...
medium viewer WebP before other retained runs, so a later click normally uses the
browser HTTP cache without keeping every decoded large bitmap in memory.
The cache-busted publication version means new products fetch afresh while
unchanged products remain reusable. The warmup is skipped for Save-Data and 2G
connections. The full-screen viewer can move left/right through every image in
the current service, including images from other retained runs.
